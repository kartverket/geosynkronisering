    ...
    public class SqlServerSpatialChangelog : SpatialDbChangelog
    {
                    ...
                    while (dr.Read())
                    {
                        ...
                        if (transType.Equals("D"))
                        {
                            ...
                            if (tempOptimizedChangeLog.Contains(gmlId))
                            {
                                ...
                                Logger.Info("SqlServerSpatialChangelog.FillOptimizedChangeLog. transType D, gmlId:{0} tempTransType:{1}", gmlId, tempTransType);
                                if (tempTransType.Equals("U"))
                                {
                                    tempOptimizedChangeLog.Remove(gmlId);
                                    Logger.Info("SqlServerSpatialChangelog.FillOptimizedChangeLog. transType D, check for UPDATE, REMOVED gmlId:{0} tempTransType:{1}", gmlId, tempTransType);
                        ...
                        else
                        {
                            ...
                            // 20201119-Leg: Turn off optimization for INSERT to prevent problems with delete in another portion
                            // If not, we can risk to send an illegal delete
                            //if (transType.Equals("I") && tempOptimizedChangeLog.Contains(gmlId))
                            //{
                            //    // test only
                            //    tempOptimizedChangeLog.Add(gmlId, new OptimizedChangeLogElement(gmlId, transType, changelogId));
                            //    Logger.Info("SqlServerSpatialChangelog.FillOptimizedChangeLog INSERT where changelog already contains gmlId. gmlId:{0}", gmlId );
                            //}
        ...
        private void PrepareChangeLogQuery(SqlConnection conn, ref SqlCommand command, int startChangeId, Int64 endChangeId, int datasetId)
        {
            ...
            try
            {
                ...
                Logger.Info("SqlServerSpatialChangelog.PrepareChangeLogQuery sqlSelectGmlIds: {0} startChangeId:{1} endChangeId:{2} datasetId:{3} ", sqlSelectGmlIds, startChangeId, endChangeId, datasetId);
