...
namespace Kartverket.Geosynkronisering.ChangelogProviders
{
    public abstract class SpatialDbChangelog : IChangelogProvider
    {
        ...
        public OrderChangelog GenerateInitialChangelog(int datasetId)
        {
            ...
            string destPath = Path.Combine(Utils.BaseVirtualAppPath, destFileName);
            ...
            using (geosyncEntities db = new geosyncEntities())
            {
                ...
                try
                {
                    ...
                    string tmpzipFile = Path.Combine(Path.GetTempPath(), zipFile);
                    ...
                    if (!ftpTool.UploadFileToFtp(tmpzipFile, ftpUrl, ftpUser, ftpPwd))
                        throw new Exception("Could not upload file to FTPServer!");
        ...
        public OrderChangelog CreateChangelog(int startIndex, int count, string todoFilter, int datasetId)
        {
            using (geosyncEntities db = new geosyncEntities())
            {
                ...
                _baseVirtualPath = Utils.BaseVirtualAppPath;
        ...
        public OrderChangelog _OrderChangelog(int startIndex, int count, string todoFilter, int datasetId)
        {
            ...
            using (geosyncEntities db = new geosyncEntities())
            {
                ...
                try
                {
                    MakeChangeLog(startIndex, count, PDbConnectInfo, _pWfsUrl, _baseVirtualPath + destFileName + ".xml",
                        datasetId);
                }
                catch (Exception ex)
                {
                    ...
                    Logger.ErrorException(
                        string.Format("Failed to make Change Log {0}", _baseVirtualPath + destFileName + ".xml"), ex);
                    throw ex;
                }
                ...
                try
                {
                    inFile = _baseVirtualPath + destFileName + ".xml";
                    zipFile = destFileName + ".zip";
        ...
        private XElement BuildChangelogRoot(int datasetId)
        {
            ...
            XElement changelogRoot =
                new XElement(nsChlogf + "TransactionCollection",
                    new XAttribute("timeStamp", DateTime.Now.ToString("yyyy-MM-ddTHH:mm:ss.ffz")),
