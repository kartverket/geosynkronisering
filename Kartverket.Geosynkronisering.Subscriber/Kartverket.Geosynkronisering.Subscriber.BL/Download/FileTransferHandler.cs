...
namespace Kartverket.Geosynkronisering.Subscriber.BL
{
    public class FileTransferHandler
    {
        ...
        private string m_fullPackageName;
        ...
        protected class AsynchronousFtpUpLoader
        {
            ...
            public void downloadFileFromFTPServer(string SourceFileName, string TargetFilename, string user = null,
                string password = null)
            {
                ...
                if (SourceFileName.Contains(@"\"))
                    file = SourceFileName.Substring(SourceFileName.LastIndexOf(@"\") + 1);
                else file = SourceFileName;
                ...
                request.BeginGetResponse(EndGetStreamCallbackDownload,
                    state
                    );
                ...
                OnFileProgressChanged(new ProgressEventArgs(state.FileName, 1, 1,
                    string.Format("The operation completed - {0}", state.StatusDescription), state.status));
            }
            ...
            private void EndGetStreamCallbackDownload(IAsyncResult ar)
            {
                ...
                try
                {
                    ...
                    using (stream = File.Create(state.FileName))
                    {
                        do
                        {
                            ...
                            count += readBytes;
                            OnFileProgressChanged(new ProgressEventArgs(state.FileName, 1, 1,
                                string.Format("Writing {0} bytes to the stream.", count), state.status));
                        } while (readBytes != 0);
                    }
                    // IMPORTANT: Close the request stream before sending the request.
                    requestStream.Close();
                    // Asynchronously get the response to the upload request.
                    state.Request.BeginGetResponse(EndGetResponseCallback,
                        state
                        );
                }
                ...
                catch (Exception e)
                {
                    ...
                    OnFileProgressChanged(new ProgressEventArgs(state.FileName, 1, 1,
                        string.Format("Could not get the request stream. \n{0}", e.Message), state.status,
                        true));
            ...
            public bool FileExistsOnServer(string fileName)
            {
                ...
                string file;
                if (fileName.Contains(@"\")) file = fileName.Substring(fileName.LastIndexOf(@"\") + 1);
                else file = fileName;
                OnFileProgressChanged(new ProgressEventArgs(fileName, 1, 1,
                    string.Format("Check if file exists: {0}{1}", "Looking for file:", file), ftpStatus.processing));
                ...
                try
                {
                    ...
                    OnFileProgressChanged(new ProgressEventArgs(fileName, 1, 1,
                        string.Format("Check file status: {0}", response.StatusDescription), ftpStatus.processing));
                }
        ...
        public bool DownloadFileFromFtp(string localFileName, string ftpFileName, string ftpserver, string user,
            string password)
        {
            ...
            try
            {
                ftp.FileProgressChanged += FileProgressChanged;
                if (!ftp.FileExistsOnServer(ftpFileName))
                    throw new Exception(string.Format("File {0} is not found on FTP server {1}.", ftpFileName, ftpserver));
            ...
            catch (Exception ex)
            {
                string preMessage = string.Format(
                    "Error downloading file: {0} from ftp server {1} to the location:{2}.", ftpFileName, ftpserver,
                    localFileName);
        ...
        public class ProgressEventArgs : EventArgs
        {
            ...
            public ProgressEventArgs(string FileName, int TotalFiles, int CurrentFile, string errorMsg, ftpStatus status,
                bool Error = false)
            {
                m_fileName = FileName;
