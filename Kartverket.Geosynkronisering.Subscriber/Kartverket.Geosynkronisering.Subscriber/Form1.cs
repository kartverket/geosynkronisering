    ...
    public partial class Form1 : Form
    {
        ...
        private void buttonSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Fill the cboDatasetName comboBox with the dataset names
                if (cboDatasetName.Items.Count > 0)
                {
                    cboDatasetName.SelectedIndex = 0;
                }
                FillComboBoxDatasetName();
        ...
        private void buttonUpdateSelectedCells_Click(object sender, EventArgs e)
        {
            foreach (DataGridViewCell cell in dgDataset.SelectedCells)
            {
                cell.Value = textBoxCellValue.Text;
            }
        }

        private void dgDataset_SelectionChanged(object sender, EventArgs e)
        {
            buttonUpdateSelectedCells.Enabled = dgDataset.SelectedCells.Count > 1;
            textBoxCellValue.Enabled = buttonUpdateSelectedCells.Enabled;
        }
    }
}